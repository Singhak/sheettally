function $(c,t,a,s,e,i,p="Valued Customer",g=""){const d=e.currency==="INR",n=d?"₹":"$",h=d?`INV-${Math.floor(1e5+Math.random()*9e5)}`:`RFQ-${Math.floor(1e5+Math.random()*9e5)}`,v=d?new Date().toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"}):new Date().toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}),b=i.upiId||"shop@upi",m=e.finalTotalWithGst,l=Math.round(m/2*100)/100,x=`upi://pay?pa=${encodeURIComponent(b)}&pn=${encodeURIComponent(i.shopName)}&am=${l.toFixed(2)}&cu=INR&tn=${encodeURIComponent(h)}`,y=`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(x)}`,u=d?`Dim: ${t.boundingBoxWidthMm.toFixed(0)} × ${t.boundingBoxHeightMm.toFixed(0)} mm | Thick: ${s.thicknessMm} mm<br>Grade: ${a} (${s.gaugeName}) | Gas: ${i.assistGas.toUpperCase()}<br>Cut Length: ${(t.totalCutLengthMm/1e3).toFixed(2)} m | Pierces: ${t.pierceCount} / pc`:`Dim: ${t.boundingBoxWidthInches.toFixed(2)}" × ${t.boundingBoxHeightInches.toFixed(2)}" | Gauge: ${s.gaugeName}<br>Grade: ${a} | Gas: ${i.assistGas.toUpperCase()}<br>Cut Length: ${t.totalCutLengthInches.toFixed(1)} in | Pierces: ${t.pierceCount} / pc | Est. Weight: ${t.theoreticalWeightLbs.toFixed(2)} lbs`;return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <title>${d?"Proforma Invoice":"Commercial Quotation"} - ${h}</title>
  <style>
    ...
    <div class="header-row">
      <div style="display: flex; align-items: center; gap: 16px;">
        ...
        <div>
          ...
          <div class="shop-meta">
            ...
            <div>Phone: ${i.shopPhone||(d?"+91 98220 12345":"(216) 555-0198")} ${d?`| UPI: <strong>${b}</strong>`:""}</div>
      ...
      <div class="invoice-tag">
        ...
        <div class="invoice-meta">
          <div>Ref No: <strong>${h}</strong></div>
...
</html>`}function _(c,t){const a=t.currency==="INR",s=a?"₹":"$",e=c.shopSettings,i=`PRJ-${Math.floor(1e5+Math.random()*9e5)}`,p=a?new Date().toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"}):new Date().toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}),g=e.upiId||"shop@upi",d=t.finalTotalWithGst,n=Math.round(d/2*100)/100,h=`upi://pay?pa=${encodeURIComponent(g)}&pn=${encodeURIComponent(e.shopName)}&am=${n.toFixed(2)}&cu=INR&tn=${encodeURIComponent(i)}`,v=`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(h)}`;let b="";for(let m=0;m<c.items.length;m++){const l=c.items[m],x=l.partGeometry.boundingBox,u=l.partGeometry.units==="inch"||!a?`${(x.width/(l.partGeometry.units==="inch"?1:25.4)).toFixed(2)}" × ${(x.height/(l.partGeometry.units==="inch"?1:25.4)).toFixed(2)}"`:`${x.width.toFixed(0)} × ${x.height.toFixed(0)} mm`,o=a?`${(l.metrics.totalCutLengthMm/1e3).toFixed(1)} m`:`${l.metrics.totalCutLengthInches.toFixed(1)} in`;b+=`
      <tr>
